...
use std::{borrow::Cow, fmt::Debug, io::Read, io::Write, time::Duration};
...
pub trait SomePacketHeader: Sized {
    ...
    fn write_to<W: Write, B: ByteOrder>(
        &self,
        writer: &mut W,
        ts_resolution: TsResolution,
    ) -> ResultParsing<()>;
...
pub trait SomePacket<'a> {
    ...
    fn from_reader<R: Read, B: ByteOrder>(
        reader: &mut R,
        ts_resolution: TsResolution,
    ) -> ResultParsing<Self::Item>;
    fn to_owned(&self) -> Self::Item;
    fn from_slice<B: ByteOrder>(
        slice: &'a [u8],
        ts_resolution: TsResolution,
    ) -> ResultParsing<(&'a [u8], Self::Item)>;
...
impl SomePacketHeader for VppPacketHeader {
    ...
    fn from_slice<B: ByteOrder>(
        mut slice: &[u8],
        ts_resolution: TsResolution,
    ) -> ResultParsing<(&[u8], VppPacketHeader)> {
        //Header len
        if slice.len() < 16 {
